"""E(n)-equivariant GNN controller (from facebookresearch/adjoint_samplers / DEM).

Accepts positions of shape ``(B, N, D)`` so it matches the rest of this repo.
"""
...
class EGNN_dynamics(nn.Module):
    ...
    def forward(self, t, pos, return_potential: bool = False):
        n_batch = pos.shape[0]
        xs = pos.reshape(n_batch, -1)
        ...
        edges = [edges[0].to(pos.device), edges[1].to(pos.device)]
        x = xs.reshape(n_batch * self._n_particles, self._spatial_dim).clone()
        h = torch.ones(n_batch, self._n_particles, device=pos.device)
...
class E_GCL(nn.Module):
    ...
    def coord_model(
        ...
        if self.agg_type == "sum":
            ...
            raise ValueError(f"Unknown coordinates aggregation type: {self.agg_type}")
        return coord + agg
